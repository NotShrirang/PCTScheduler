...
def schedule(raw_schedule, YEAR, MONTH_NAME):
    ...
    INT_TO_DAYS = {0: 'Monday', 1: 'Tuesday', 2: 'Wednesday', 3: 'Thursday', 4: 'Friday', 5: 'Saturday', 6: 'Sunday'}
    ...
    raw_group3_schedule = pd.DataFrame(raw_schedule[['Group 3 (1800 - 2300)', 'Unnamed: 1', 'Unnamed: 2', 'Unnamed: 3', 'Unnamed: 4']])
    Promotional_INDEX = raw_group3_schedule.loc[raw_group3_schedule['Group 3 (1800 - 2300)']=='Special psychological class for all. Parents are also invited'].index
    if len(Promotional_INDEX) != 0:
        raw_group3_schedule.drop(range(Promotional_INDEX[0], raw_group3_schedule.index[-1]), inplace=True)
    Homework_INDEX = raw_group3_schedule.loc[raw_group3_schedule['Group 3 (1800 - 2300)']=='Exercise Reviews'].index
    if len(Homework_INDEX) != 0:
        raw_group3_schedule.drop(range(Homework_INDEX[0], raw_group3_schedule.index[-1]), inplace=True)
    raw_group3_schedule.rename(columns={'Group 3 (1800 - 2300)': "Date", 'Unnamed: 1': "Day", 'Unnamed: 2': "IST Time", 'Unnamed: 3': 'Coach Name', 'Unnamed: 4': 'Topic'}, inplace=True)
    ...
    Grp3_schedule.drop([Grp3_schedule.index[-1], Grp3_schedule.index[-2]], inplace=True)
    null_list = Grp3_schedule.loc[Grp3_schedule['Date'].isnull()].index
    Grp3_schedule.drop(null_list, inplace=True)
    del null_list

    Grp1_schedule = raw_schedule[['Group 1', 'Unnamed: 6', 'Unnamed: 7', 'Unnamed: 8', 'Unnamed: 9']].drop([0], axis=0)
    Grp1_schedule = Grp1_schedule.rename(columns={'Group 1': "Date", 'Unnamed: 6': "Day", 'Unnamed: 7': "IST Time", 'Unnamed: 8': 'Coach Name', 'Unnamed: 9': 'Topic'})
    SEMINARS_INDEX = Grp1_schedule.loc[Grp1_schedule['Day']=='SEMINARS G2-3'].index[0]
    Grp1_schedule.drop(range(SEMINARS_INDEX-2, Grp1_schedule.index[-1]-1), inplace=True)
    ...
    Grp2_schedule = raw_schedule[['Group 1', 'Unnamed: 6', 'Unnamed: 7', 'Group 2', 'Unnamed: 11']].drop([0], axis=0)
    Grp2_schedule = Grp2_schedule.rename(columns={'Group 1': "Date", 'Unnamed: 6': "Day", 'Unnamed: 7': "IST Time", 'Group 2': 'Coach Name', 'Unnamed: 11': 'Topic'})
    Grp2_schedule.drop(range(SEMINARS_INDEX-2, Grp2_schedule.index[-1]-1), inplace=True)
    ...
    Elite_schedule = raw_schedule[['Elite Group (2300+)', 'Unnamed: 13', 'Unnamed: 14', 'Unnamed: 15', 'Unnamed: 16']].drop([0], axis=0)
    Elite_schedule = Elite_schedule.rename(columns={'Elite Group (2300+)': "Date", 'Unnamed: 13': "Day", 'Unnamed: 14': "IST Time", 'Unnamed: 15': 'Coach Name', 'Unnamed: 16': 'Topic'})
    null_list = Elite_schedule.loc[Elite_schedule['Date'].isnull()].index
    Elite_schedule.drop(null_list, inplace=True)
    del null_list

    Grp1_schedule.insert(len(Grp1_schedule.columns), column='Group', value=["Group 1"]*len(Grp1_schedule.index))
    Grp2_schedule.insert(len(Grp2_schedule.columns), column='Group', value=["Group 2"]*len(Grp2_schedule.index))
    Grp3_schedule.insert(len(Grp3_schedule.columns), column='Group', value=["Group 3"]*len(Grp3_schedule.index))
    Elite_schedule.insert(len(Elite_schedule.columns), column='Group', value=["Elite"]*len(Elite_schedule.index))

    final_schedule = pd.concat([Grp1_schedule, Grp2_schedule, Grp3_schedule, Elite_schedule], axis=0, ignore_index=True, keys=['Freshers Group', 'Group 1', 'Group 2', 'Group 3', 'Elite Group'])
    ...
    REVIEW_TESTS = {"CLASS TYPE" : "Review Tests", "GROUP" : "", "COACH" : "", "DATE" : "", "TIME": "", "TITLE" : "Review Test"}
    TRAINING_GAMES = {"CLASS TYPE" : "Training Games", "GROUP" : "", "COACH" : "", "DATE" : "", "TIME": "", "TITLE" : "Training Games - 4 rounds - 10 + 2"}
    RAPID_TOURNAMENTS = {"CLASS TYPE" : "Tournaments", "GROUP" : "", "COACH" : "", "DATE" : "", "TIME": "", "TITLE" : "Rapid Tournament - 4 rounds - 10 + 2"}
    BLITZ_TOURNAMENTS = {"CLASS TYPE" : "Tournaments", "GROUP" : "", "COACH" : "", "DATE" : "", "TIME": "", "TITLE" : "Blitz Tournament - 7 rounds - 3 + 2"}
    SEMINAR = {"CLASS TYPE" : "Seminars", "GROUP" : "", "COACH" : "", "DATE" : "", "TIME": "", "TITLE" : ""}
    ...
    def ist_to_est(ist_series):
        ...
        for count, entry in enumerate(list(ist_series[['TIME', 'DATE']].iloc)):
            ...
            time = entry['TIME']
            date = entry['DATE']
            date_time = datetime.datetime(date.year, date.month, date.day, time.hour, time.minute)
            est_time = date_time - diff
            day = INT_TO_DAYS[est_time.weekday()]
            est_df.loc[len(est_df.index)] = (est_time.date(), day, est_time.time())
        return est_df
    ...
    for group in GROUPS:
        ...
        est_upload_schedule.insert(0, column='EST Date', value=est_df['EST Date'])
        est_upload_schedule.insert(1, column='EST Day', value=est_df['EST Day'])
        est_upload_schedule.insert(2, column='EST Time', value=est_df['EST Time'])
